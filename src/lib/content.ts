    ...
    features: [
      {
        ...
        casual: "One environment variable disables every user-defined agent endpoint with 403 — the kill switch for regulated deployments.",
        technical: "STRICT_AGENTS_ONLY=true wires a FastAPI router-level dependency that returns 403 on every /custom-agents endpoint before the body executes — no Neo4j hit, no agent load, no runtime exposure. Read at request time so operators can flip the switch without restart. Logged at WARNING for incident-response audit. Built-in 10 specialist agents remain available.",
      },
      {
        icon: Lock,
        title: "Governed MCP Client",
        casual: "Choose how Cerid consumes external MCP servers — allow everything, allowlist specific servers, or disable entirely.",
        technical: "MCP_CLIENT_MODE supports three modes: permissive (default — every configured server callable), allowlist (only servers in MCP_CLIENT_ALLOWLIST), disabled (every external MCP call denied — kill switch). Every call emits a structured INFO log with tool + server + status + elapsed time, plus a Sentry breadcrumb. Denials happen before the wire. Per-call read of both env vars; flip without restart.",
      },
      {
        icon: Eye,
        title: "Per-Call Audit Trail",
        casual: "Every external tool call, swallowed error, and verification verdict produces a structured log entry — provable behavior under audit.",
        technical: "Sentry breadcrumbs on every external-MCP call, swallowed_error counters via core.utils.swallowed (Redis sorted-set tracking surfaced at /health.swallowed_errors_last_hour), stage breadcrumbs on every internal LLM call (contract-tested), request_id contextvar propagation across async boundaries, tenant_id contextvar for multi-tenant isolation.",
      },
      {
        icon: Lock,
        title: "Privacy-First Telemetry",
        casual: "Sentry, ChromaDB telemetry, and external reporting are off by default. Opt in only when you decide.",
        technical: "ENABLE_SENTRY env var defaults off — Sentry SDK initialization is conditional, no DSN required for normal operation. ChromaDB anonymized_telemetry=false in compose. Optional Fernet encryption-at-rest via ENABLE_ENCRYPTION. Setup-wizard env writes routed to a host bind-mounted .env so credentials persist across container rebuild without leaving the host.",
      },
    ],
  },
  {
...
export const PLANS: Plan[] = [
  {
    ...
    features: [
      ...
      "Streaming hallucination detection (4 claim types, NLI-gated)",
  ...
  },
  {
    ...
    features: [
      "Everything in Pro",
      "Compliance-assistance pack: documented patterns for Strict-Agents Mode + Governed MCP rollout",
      "Curated model registry with foreign-origin model exclusion guidance",
      "Multi-user JWT auth + tenant isolation + tenant_id contextvar propagation",
